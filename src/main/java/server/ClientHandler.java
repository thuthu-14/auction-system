...
public class ClientHandler implements Runnable {
    ...
    private void handleLogin(Message message) throws IOException, ClassNotFoundException {
        ...
        try {
            ...
            System.out.println("🔥 LOGIN SET currentUser = " + user.getUsername() + " role=" + user.getRole());
    ...
    private void handleGetAllUsers(Message message) throws IOException {
        ...
        try {
            ...
            // ✅ FIX QUAN TRỌNG: check role an toàn hơn (tránh enum/string sai)
            ...
            Message response = new Message(MessageType.SUCCESS, "SUCCESS", "GET_USERS_SUCCESS");
            response.setData(users);

            sendMessage(response);

            System.out.println("✔ GET_ALL_USERS success size = " + users.size());

        } catch (Exception e) {
            e.printStackTrace();
            sendError("Server lỗi: " + e.getMessage());
        }
    }
    ...
    private void handleUpgradeSeller(Message message) throws IOException, ClassNotFoundException {
        ...
        try {
            ...
            // ✅ Lấy shop info từ request
            @SuppressWarnings("unchecked")
            Map<String, String> sellerData = (Map<String, String>) message.getData();

            String shopName = sellerData.get("shopName");
            String shopPhone = sellerData.get("shopPhone");
            String shopAddress = sellerData.get("shopAddress");
            String shopEmail = sellerData.get("shopEmail");

            // ✅ Nâng cấp với shop info
    ...
    private void handleGetSellerAuctions(Message message) throws IOException, ClassNotFoundException {
        ...
        try {
            ...
            List<Auction> sellerAuctions =
                    AuctionService.getAuctionsBySeller(regularUser.getUserId());

            Message response = new Message(
                    MessageType.SUCCESS,
                    "SUCCESS",
                    "Seller auctions retrieved"
            );

            response.setData(sellerAuctions);
            sendMessage(response);
